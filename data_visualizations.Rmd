...
# Plot
plot1 <- ggplot(exprs_num_df_combinations, aes(x = Exprs.Source, y = `Exprs.Count`, fill = Color_Group)) +
  geom_bar(stat = "identity", width = 0.8) +
  geom_text(aes(label = `Exprs.Count`), hjust = -0.2, size = 3) +
  scale_fill_manual(
...
test_pData[test_pData$STUDY == "Ribolution_ukd2", ]$BCR_STATUS <- 
  test_pData[test_pData$STUDY == "Ribolution_ukd2", ]$DOD_STATUS / 100

# Create BCR Data subset
all_pData_MONTH_TO_BCR <- rbind(train_pData[, c("MONTH_TO_BCR", "STUDY", "BCR_STATUS")] %>%
                              select(MONTH_TO_BCR, STUDY, BCR_STATUS) %>%
...
all_pData_AGE <- all_pData_AGE %>%
  ...
  complete(
    STUDY = factor(rename_map, levels = rename_map),
    fill = list(AGE = NA, Group = "NA")
  ) %>%
  mutate(Group = factor(Group, levels = c("Group 1", "Group 2", "NA")))
