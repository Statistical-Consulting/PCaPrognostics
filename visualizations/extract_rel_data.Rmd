...
for(i in 1:2){
  all_pData_common_cols <- rbind(all_pData_common_cols, cohorts_b[[i]]$pData[common_cols])
}

```
```{r load data, include=FALSE}
mean_observation_time_no_event_a <- 
  mean(na.omit(all_pData_common_cols[a_pData$BCR_STATUS == 0,c("MONTH_TO_BCR")]))

mean_observation_time_no_event_10 <- mean(na.omit(cohorts_b[[1]]$
                                      pData[common_cols][cohorts_b[[1]]$pData[common_cols]$BCR_STATUS == 0,c("MONTH_TO_BCR")]))

mean_observation_time_no_event_11 <- median(na.omit(cohorts_b[[2]]
                                      $pData[common_cols][cohorts_b[[2]]$pData[common_cols]$DOD_STATUS == 0,c("MONTH_TO_DOD")]))

mean_observation_to_BCR_a <- 
  mean(na.omit(all_pData_common_cols[a_pData$BCR_STATUS == 1,c("MONTH_TO_BCR")]))

mean_observation_to_BCR_10 <- mean(na.omit(cohorts_b[[1]]$
                                      pData[common_cols][cohorts_b[[1]]$pData[common_cols]$BCR_STATUS == 1,c("MONTH_TO_BCR")]))

mean_observation_to_DOD_11 <- median(na.omit(cohorts_b[[2]]
                                      $pData[common_cols][cohorts_b[[2]]$pData[common_cols]$DOD_STATUS == 1,c("MONTH_TO_DOD")]))
...
test <- cohorts_b[[1]]$pData[common_cols]
```
```{r load data, include=FALSE}

sum(is.na(all_pData$AGE))
t <- as.numeric(all_pData$PRE_OPERATIVE_PSA)
sum(is.na(t))
t
```