...
subset_splits_results <- splits_results_cBoost[splits_results_cBoost$dataset %in% c("AutoEncoder", "Intersection", "Imputed"), ] %>%filter(dataset %in% c("Intersection", "Imputed"))
...
splits_best_coxph_all_data <- splits_results_coxph_all_data %>%
  ...
  ungroup()
colnames(splits_best_coxph) <- c("...1", "test_cohort", "ci_se", "ci", "model", "dataset")
colnames(splits_best_coxph_all_data) <- c("...1", "test_cohort", "ci_se", "ci", "model", "dataset")
...
colnames(splits_best_rsf_all_data)[colnames(splits_best_rsf_all_data) == "testing_cohort"] <- "test_cohort"
...
```{r correlation ebtween model performance and % of events }

# Berechnung der Medianwerte pro Test-Cohort
...
merged_data <- bcr_summary %>%
  inner_join(median_data, by = c("cohort" = "test_cohort"))
#used spearman because of the small data set and interest in monoton relation
bcr_performance_cor <- cor(merged_data$BCR_Percentage, merged_data$median_ci, method = "spearman")

#### Plot Goal: Using median performance(median w.r.t. to the input data set) of each model type
# on each cohort to check whether to get the correlation for the different model types 
splits_results_coxph_ci <- splits_results_coxph
splits_results_coxph_ci$ci<- splits_results_coxph$ci_min
colnames(splits_results_coxph_ci)[2] <- "test_cohort"
splits_results_rsf_renamed <- splits_results_rsf
colnames(splits_results_rsf_renamed)[2] <-  "test_cohort"

models_list <- list(
  "Gradient Boosting" = splits_results_cBoost,
  "DeepSurv" = splits_results_DeepSurv,
  "RSF" = splits_results_rsf_renamed,
  "Cox-PASNet" = splits_results_coxPas,
  "Priority Lasso" = splits_results_prio,
  "ffpe-Score" = splits_results_score,
  "Cox PH" = splits_results_coxph_ci
)

compute_median_ci <- function(df) {
  df %>%
    group_by(test_cohort) %>%
    summarise(median_ci = median(ci, na.rm = TRUE), .groups = "drop")
}

median_ci_list <- map(models_list, compute_median_ci)

median_ci_data <- reduce(median_ci_list, full_join, by = "test_cohort")

colnames(median_ci_data)[-1] <- names(models_list)

median_ci_data <- median_ci_data %>%
  mutate(test_cohort = recode(test_cohort, !!!rename_map))

merged_median_ci_data <- median_ci_data %>%
  inner_join(bcr_summary, by = c("test_cohort" = "cohort"))

model_columns <- colnames(merged_median_ci_data)[2:(ncol(merged_median_ci_data) - 1)]
...
correlations <- sapply(model_columns, function(model) {
  cor(merged_median_ci_data[[model]], merged_median_ci_data$BCR_Percentage, method = "pearson", use = "complete.obs")
})
...
ggplot(correlation_df, aes(x = reorder(model, correlation), y = correlation)) +
  geom_bar(stat = "identity", fill = "#00706d") +  # Rote Balken für negative Werte
  coord_flip() +  # Horizontal drehen (damit Balken nach unten zeigen)
  labs(
